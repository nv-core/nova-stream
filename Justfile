export IMAGE_NAME := env("IMAGE_NAME", "nova-stream")
export DEFAULT_TAG := env("DEFAULT_TAG", "stable")
export PODMAN := env("PODMAN", "podman")
export REPO_ORG := env("GITHUB_REPOSITORY_OWNER", "nv-core")
...
#
# nova-stream builds two tracks from two Containerfiles:
#   just build                          -> stable track (Fedora 43 / Kodi 21.3)
#   just build nova-stream beta Containerfile.beta
#                                       -> beta track   (Fedora 44 / Kodi 22.0b1)
build $target_image=IMAGE_NAME $tag=DEFAULT_TAG $containerfile="Containerfile":
    ...
    # Read the Fedora major version from the selected Containerfile (single
    # source of truth). The base image itself is pinned in its FROM line.
    ...
    LABELS+=("--label" "org.opencontainers.image.description=${IMAGE_DESC:-Kodi media appliance built on Fedora Atomic (bootc)}")
    ...
    LABELS+=("--label" "io.artifacthub.package.keywords=${IMAGE_KEYWORDS:-bootc,ublue,kodi,htpc,mediacenter,libreelec}")
